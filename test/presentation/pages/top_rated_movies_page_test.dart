...
void main() {
  ...
  testWidgets('Page should display center progress bar when loading',
        ...
        when(() => mockTopRatedMoviesBloc.add(GetTopRatedMoviesEvent())).thenAnswer((invocation) {});
        when(() => mockTopRatedMoviesBloc.state).thenAnswer((invocation) => TopRatedMoviesLoading());
  ...
  testWidgets('Page should display ListView when data is loaded',
          (WidgetTester tester) async {
        when(() => mockTopRatedMoviesBloc.add(GetTopRatedMoviesEvent())).thenAnswer((invocation) {});
        when(() => mockTopRatedMoviesBloc.state).thenAnswer((invocation) => TopRatedMoviesLoaded(<Movie>[]));
  ...
  testWidgets('Page should display text with message when Error',
          (WidgetTester tester) async {
        when(() => mockTopRatedMoviesBloc.add(GetTopRatedMoviesEvent())).thenAnswer((invocation) {});
        when(() => mockTopRatedMoviesBloc.state).thenAnswer((invocation) => TopRatedMoviesError('Error Message'));
