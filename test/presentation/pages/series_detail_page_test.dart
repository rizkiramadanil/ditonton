...
class FakeSeriesDetailEvent extends Fake implements SeriesDetailState {}

class FakeSeriesDetailState extends Fake implements SeriesDetailEvent {}
...
void main() {
  ...
  testWidgets(
      'Watchlist button should display add icon when series not added to watchlist',
        ...
        when(() => mockSeriesDetailBloc.add(GetSeriesDetailEvent(1))).thenAnswer((invocation) {});
        when(() => mockSeriesDetailBloc.state).thenAnswer((invocation) => SeriesDetailLoaded(testSeriesDetail));
        when(() => mockSeriesRecommendationBloc.add(GetSeriesRecommendationEvent(1))).thenAnswer((invocation) {});
        when(() => mockSeriesRecommendationBloc.state).thenAnswer((invocation) => SeriesRecommendationLoaded(testSeriesList));
        when(() => mockWatchlistSeriesBloc.add(GetStatusSeriesEvent(1))).thenAnswer((invocation) {});
        when(() => mockWatchlistSeriesBloc.state).thenAnswer((invocation) => WatchlistSeriesStatusLoaded(false));
  ...
  testWidgets(
      'Watchlist button should display check icon when series is added to wathclist',
        ...
        when(() => mockSeriesRecommendationBloc.state).thenAnswer((invocation) => SeriesRecommendationLoaded(testSeriesList));
        when(() => mockWatchlistSeriesBloc.add(GetStatusSeriesEvent(1))).thenAnswer((invocation) {});
        when(() => mockWatchlistSeriesBloc.state).thenAnswer((invocation) => WatchlistSeriesStatusLoaded(true));
