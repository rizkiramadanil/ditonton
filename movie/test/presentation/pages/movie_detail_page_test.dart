...
void main() {
  ...
  testWidgets(
      'Watchlist button should display add icon when movie not added to watchlist',
        ...
        when(() => mockMovieDetailBloc.state).thenReturn(MovieDetailLoaded(testMovieDetail));
        when(() => mockMovieRecommendationBloc.state).thenReturn(MovieRecommendationLoaded(testMovieList));
        when(() => mockWatchlistMoviesBloc.state).thenReturn(WatchlistMoviesStatusLoaded(false));
  ...
  testWidgets(
      'Watchlist button should display check icon when movie is added to wathclist',
          (WidgetTester tester) async {
            when(() => mockMovieDetailBloc.state).thenReturn(MovieDetailLoaded(testMovieDetail));
            when(() => mockMovieRecommendationBloc.state).thenReturn(MovieRecommendationLoaded(testMovieList));
            when(() => mockWatchlistMoviesBloc.state).thenReturn(WatchlistMoviesStatusLoaded(true));
